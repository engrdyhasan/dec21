 ...
 **add_filter()
 */
function hassan_extend_excerpt_length($length){
  if (is_author()) {
    return 22;
  }else{
    return 85;
...
add_filter('excerpt_length', 'hassan_extend_excerpt_length');

function hassan_excerpt_change_dots($more){
  return ' ..more ...';
}
add_filter('excerpt_more', 'hassan_excerpt_change_dots');
/**
 **add actions
 **added by hassan
 **add_action()
...
add_action('init', 'hassan_register_menu');
